...
namespace VContainer.Internal
{
    ...
    readonly struct DependencyInfo
    {
        ...
        public readonly IRegistration Dependency;
        readonly IRegistration owner;
        readonly object method; // ctor or method
        readonly ParameterInfo param; // param or field or prop

        public DependencyInfo(IRegistration dependency)
        {
        ...
        public DependencyInfo(IRegistration dependency, IRegistration owner, ConstructorInfo ctor, ParameterInfo param)
        {
            Dependency = dependency;
            this.owner = owner;
            method = ctor;
        ...
        public DependencyInfo(IRegistration dependency, IRegistration owner, MethodInfo method, ParameterInfo param)
        {
            Dependency = dependency;
            this.owner = owner;
            this.method = method;
        ...
        public DependencyInfo(IRegistration dependency, IRegistration owner, FieldInfo field)
        {
            Dependency = dependency;
            this.owner = owner;
            method = field;
        ...
        public DependencyInfo(IRegistration dependency, IRegistration owner, PropertyInfo prop)
        {
            Dependency = dependency;
            this.owner = owner;
            method = prop;
    ...
    static class TypeAnalyzer
    {
        ...
        static void CheckCircularDependencyRecursive(DependencyInfo current, Registry registry, Stack<DependencyInfo> stack)
        {
            ...
            {
                if (injectTypeInfo.InjectConstructor.HasValue)
                {
                    foreach (var x in injectTypeInfo.InjectConstructor.Value.ParameterInfos)
                    {
                        if (registry.TryGet(x.ParameterType, out var parameterRegistration, injectTypeInfo.Type))
                        {
                            CheckCircularDependencyRecursive(new DependencyInfo(parameterRegistration, current.Dependency, injectTypeInfo.InjectConstructor.Value.ConstructorInfo, x), registry, stack);
                        }
                    }
                }
                ...
                if (injectTypeInfo.InjectFields != null)
                {
                    ...
                    {
                        if (registry.TryGet(x.FieldType, out var fieldRegistration, injectTypeInfo.Type))
                        {
                ...
                if (injectTypeInfo.InjectProperties != null)
                {
                    ...
                    {
                        if (registry.TryGet(x.PropertyType, out var propertyRegistration, injectTypeInfo.Type))
                        {
