...
namespace VContainer.Diagnostics
{
    public sealed class DiagnosticsCollector
    {
        ...
        public void TraceBuild(IRegistrationBuilder registrationBuilder, IRegistration registration)
        {
            lock (diagnosticsInfos)
            {
                foreach (var x in diagnosticsInfos)
                {
                    if (x.RegisterInfo.RegistrationBuilder.ImplementationType == registrationBuilder.ImplementationType)
                    {
